...
class Quantity:
    ...
    def __init__(self, value=0, **kwargs):
        self.value = value + sum(self.units[symbol].scale*value for symbol, value in kwargs.items())

    def __hash__(self):
        return hash((self.value, type(self)))
    
    def to_string(self, unit: Unit, float_fmt: str='.6f') -> str:
        return f"{self.value/unit.scale:{float_fmt}} {unit.symbol}"
    ...
    def __format__(self, fmt: str) -> str:
        float_fmt, unit_symbol = re.match('(\d*\.\d*)(.+)', fmt).groups()
